...
class Menu:
    ...
    def __init__(self, master): # making the screen?
        self.master = master
        self.master.geometry("400x300")
        self.show_widgets()

    def show_widgets(self): # showing the screen menu
    ...
    def create_button_for_script_mode(self, text, _class): #creat button for the script mode
        "Button that creates a new window"
        tk.Button(
            self.frame, text=text,
            command=lambda: self.open_file()).pack()

    def open_file(self): #choosing which file to open
        self.root = tk.Tk()
        self.root.withdraw()
        self.file_path = filedialog.askopenfilename()
        print(self.file_path)

    def create_button(self, text, _class): #creat button for new window.
        "Button that creates a new window"
        tk.Button(
            self.frame, text=text,
            command=lambda: self.new_window(_class)).pack() #creat button for painter.
...
class Win_Motor_Control(Menu):    # motor control      
...
class Win_Painter(Menu):        # painter option.
    def __init__(self, master):
        self.master = master
...
class Win_Stepper_Calibration(Menu):    #calibrate the stepper to initial position.
    def __init__(self, master):
        self.master = master
...
class Win_Script(Menu):  #Win3       script option for choosing.
    def __init__(self, master):
...
class Win_Reset_Motor(Menu):            # Reset the motor 
    def __init__(self, master):
        self.master = master
...
class Win_steps(Menu): #win steps
    def __init__(self, master):
        self.master = master
        self.master.title("Reset Motor")
        self.master.geometry("400x200+200+200")
...
def exit_menu(root): #exit from menu
    global menu_flag
    menu_flag = 0
    root.destroy()
    root.quit()

def menu_chosie1(root): #menu choise number 1
    global menu
    menu = 1
    root.destroy()
    root.quit()
def menu_chosie2(root): #menu choise number 2
    global menu
    menu = 2
    root.destroy()
    root.quit()
def menu_chosie3(root): #menu choise number 3
    global menu
    menu = 3
    root.destroy()
    root.quit()
def menu_chosie4(root): #menu choise number 4
    global menu
    menu = 4
    root.destroy()
    root.quit()
# Option 3 at menu chosie
def calabrtion_chosie(root): #Calibration choise
    global cal_step_chosie
    cal_step_chosie = 1
    root.destroy()
    root.quit()
def step_chosie(root): #Calibration choise
    global cal_step_chosie
    cal_step_chosie = 2
    root.destroy()
    root.quit()

def close_windows(root): # close the window
    global flag
    flag = 0
    root.destroy()
    root.quit()
...
def checkPostionInfo(): #checking the position 
...
def movment(): # the movement of the calibrator
...
def quitTurttle(x, y): #quit from the turtle window
    global stopTurttle
    stopTurttle = False

def fileUpload(root): #upload the file script
    global fileAmount
...
def runFile(fileNum): #running the file
    enableTX = True
    while (s.out_waiting > 0 or enableTX):
        inChar = fileNum
        bytesChar = bytes(inChar, 'ascii')
